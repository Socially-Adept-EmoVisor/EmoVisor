...
class DataAugmentor(object):
    ...
    def __call__(self, image, bb=None, shape=None):
        ...
        if(bb is None):
            ...
            scalingFactor = self.target_height / \
                image.shape[0]  # Image is square
            ...
            scale = self.rng.rand() * self.random_scaling*2 + \
                (1-self.random_scaling)  # ex: random_scaling is .25
            mat = cv2.getRotationMatrix2D(
                (center[0], center[1]), aug_rot, scale)
            image = cv2.warpAffine(
                image, mat, (self.target_width, self.target_height))

            # Transforms the shape as well using homogeneous coordinates
            if shape is not None:
                shape = np.dot(np.concatenate(
                    (shape, shape[:, 0:1]*0+1), axis=1), mat.T)

            if self.random_translation != 0:
                dx = self.rng.randint(-self.random_translation *
                                      scale, self.random_translation * scale)  # in px
                dy = self.rng.randint(-self.random_translation *
                                      scale, self.random_translation * scale)
            else:
                dx, dy = 0, 0
            ...
            # in deg.
            aug_rot = (self.rng.rand() * 2 - 1) * self.random_rotation
            aug_scale = self.rng.rand() * self.random_scaling*2 + \
                (1-self.random_scaling)  # ex: random_scaling is .25
            scale *= aug_scale

            if self.random_translation != 0:
                dx = self.rng.randint(-self.random_translation * scale,
                                      self.random_translation * scale)/center[0]  # in px
                dy = self.rng.randint(-self.random_translation *
                                      scale, self.random_translation * scale)/center[1]
            else:
                dx, dy = 0, 0
            ...
            mat = get_transform(
                center, scale, (self.target_width, self.target_height), aug_rot)[:2]
            # , borderMode= cv2.BORDER_WRAP)
            image = cv2.warpAffine(
                image, mat, (self.target_width, self.target_height))

            if shape is not None:
                mat_pts = get_transform(
                    center, scale, (self.target_width, self.target_height), aug_rot)[:2]
                shape = np.dot(np.concatenate(
                    (shape, shape[:, 0:1]*0+1), axis=1), mat_pts.T)
